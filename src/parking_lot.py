# Parking Lot Project - Arindam Tanti
import os
import time # detect time
...
# OBEJCT DETECTION USING CLASS :vehicle class, has a type and a plate number, upon creation, stores current epoch time for later fare calculation
class Vehicle:
    # vehicle Class Constructure
    def __init__(self, v_type, plate):
        self.type = v_type # users_Detail : small, medium large 
        self.plate = plate # users_Detail : number plate
        self.entry_time = time.time() # users_Detail : when the vehicle was entered

    # FUNCTION TO RETURN THE VEHICLE TYPE : return type value (int)
    def get_type(self):
        return self.type

    # There can be many more types : return type value (string)
    def get_type_string(self):
        return "Car" if self.type == 1 else "Truck" if self.type == 2 else "Motorcycle"
    
    # To get the number plate details
    def get_plate(self):
        return self.plate
...
class Space:
    ...
    # get the vehicel info : Plate, type, place where it is located in the space.
    def vehicle_info(self):
        return self.vehicle

    # Is the place is occupied or not
    def is_available(self):
        return self.occupied

# to print the rows in the space
def print_row(row):
    output = ""
    output += "|"
...
def display_row_selection():
    ...
    # !!!!! THIS IS TO CHECK THE RUNNING OPERATION SYSTEM !!!!!!! : only uncomment when running on linux machine 
    if linux == 1:
        os.system("clear")
    print(output)


# DISPLAY OF PARKED VEHILES FUNCTION : display a specified row with space selection numbers for user to choose from
...
# VEHICLE ENTRY FUNCTION : used to park a vehicle within the lot
def enter_vehicle(v_type, plate, row, space):
    global spaces, avail_spaces, total_spaces, rows 

    # condtion =>  do not allow a user to park a vehicle with a full lot, sorry our parking space is fully parked :p
    if avail_spaces == 0:
        display_lot()
    ...
    # condition : check if a specified space is already occupied, you can not park a vehicle in a same spot.
    ...
    # check if specified plate number is in the lot as a vehicle can not be parked twice, some might be playing with you.
    for uniq in spaces:
...
def read_config():
    ...
    while True:
        ...
        # if demo mode is enabled, populate lot with demo cars, otherwise, populate lot based on config
            ...
            if int(line[10:11]) == 1: # if the config file has line demo_mode=0 then it will not trigger this demo_mode() function
                demo_mode()
                break
            else:
...
def main():
    ...
    # begin accepting user commands whatever they can put here.
    command = ""
    while command != "Q":
        display_lot()
